...
def test_run_initiation_end_to_end(monkeypatch, tmp_path):
    ...
    async def exercise_flow():
        ...
        async with AsyncClient(transport=transport, base_url="http://testserver") as client:
            ...
            assert post_payload["api_specification"] == "Create a products API with CRUD endpoints"
            assert post_payload["status"] == "initiated"
            assert "id" in post_payload

            mocked_orchestration.assert_awaited_once_with("Create a products API with CRUD endpoints")

            get_response = await client.get(f"/api/v1/runs/{post_payload['id']}")
            assert get_response.status_code == 200
            get_payload = get_response.json()
            assert get_payload["id"] == post_payload["id"]
